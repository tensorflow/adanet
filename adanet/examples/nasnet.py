...
class _NASNet(adanet.BaseLearnerBuilder):
  """Builds a NASNet-A base learner for AdaNet."""
  ...
  def __init__(self, optimizer_fn, learning_rate_schedule_fn,
    ...
    Args:
      ...
      initial_learning_rate: Float learning rate to use when there are no
        ...
        best base learner is used instead.
  ...
  def build_base_learner(self, features, logits_dimension, training,
  ...
  def build_base_learner_train_op(self, base_learner, loss, var_list, labels,
    ...
    with tf.name_scope(""):
      summary.scalar("learning_rate/adanet/base_learner", learning_rate)

    onehot_labels = tf.one_hot(
        tf.reshape(labels, [-1]), base_learner.logits.shape[-1], dtype=tf.int32)

    loss = tf.losses.softmax_cross_entropy(
        onehot_labels=onehot_labels,
        logits=base_learner.logits,
        weights=1.0,
        label_smoothing=0.1)

    # TODO: Figure out how to handle auxiliary heads in a more
    # flexible way than hard-coding the loss for classification.
    if "aux_logits" in base_learner.persisted_tensors:
      loss += tf.losses.softmax_cross_entropy(
          onehot_labels=onehot_labels,
          logits=base_learner.persisted_tensors["aux_logits"],
          weights=0.4,
          label_smoothing=0.1,
          scope="aux_loss")
  ...
  @property
  def name(self):
    """Returns this base learner's name."""
...
class Generator(adanet.BaseLearnerBuilderGenerator):
  """Generates NASNet-A base learners with the configured structure."""
