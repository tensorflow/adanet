...
class Subnetwork(
  ...
  Args:
    last_layer: :class:`tf.Tensor` output or dict of string to
      ...
      type is `MATRIX`, the AdaNet algorithm takes care of computing ensemble
      mixture weights matrices (one per subnetwork) that multiply the various
      last layers of the ensemble's subnetworks, and regularize them using their
      subnetwork's complexity. This field is represented by 'h' in the AdaNet
      paper.
    logits: :class:`tf.Tensor` logits or dict of string to :class:`tf.Tensor`
      ...
      used in the ensemble's outputs if the mixture weight type is `MATRIX`,
      instead AdaNet learns its own logits (mixture weights) from the
      subnetwork's `last_layers` with complexity regularization. The logits are
      used in the ensemble only when the mixture weights type is `SCALAR` or
      `VECTOR`. Even though the logits are not used in the ensemble in some
    ...
    persisted_tensors: DEPRECATED. See `shared`. Optional nested dictionary of
      ...
      an iteration, the :class:`tf.Tensor` instances will be available to
        ...
        upon
      arbitrary :class:`tf.Tensors` from previous
  ...
  def build_mixture_weights_train_op(self, loss, var_list, logits, labels,
    ...
    Args:
      ...
      iteration_step: Integer :class:`tf.Tensor` representing the step since the
        beginning of the current iteration, as opposed to the global step.
      summary: An :class:`adanet.Summary` for scoping summaries to individual
        subnetworks in Tensorboard. Using :class:`tf.summary` within this scope will
        use this :class:`adanet.Summary` under the hood.
    ...
    Args:
      ...
      previous_ensemble_reports: List of
        ...
        the Builders composing :class:`adanet.Ensemble` from iteration t-1. The
          ...
          first
        iteration. If a :class:`adanet.subnetwork.MaterializedReport` is not
          supplied to the estimator, previous_ensemble_report is `None`.
